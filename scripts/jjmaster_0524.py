...
class stool:
    ...
    def callback1(self, msg):
        ...
        if msg.data == "sceinaro 1": #child 이고 최상단을 뽑음
            if self.bookcase == "book1" or self.bookcase =="book2" or self.bookcase == "book3" :
                stool.turtlebot3_move=True
                self.movebase_client(0.55,1.5,0)
                ...
                if self.bookcase == "book1":
                    ...
                    stool.turtlebot3_move=True   
                    self.movebase_client(1.9,1.5,0) # book 3의 좌표
                    stool.turtlebot3_move=False
                else:
            ...
            if self.bookcase == "book1" or self.bookcase == "book2" or self.bookcase == "book3" or self.bookcase == "book4" or self.bookcase == "book5" or self.bookcase == "book6" or self.bookcase == "book7" or self.bookcase == "book8" or self.bookcase == "book9":
                stool.turtlebot3_move=True   
                self.movebase_client(0.55,1.5,0) # 책장 옆 트롤리 위치
                stool.turtlebot3_move=False
            else:
                rospy.loginfo("Something is error with book num or sceinaro 2")


        elif msg.data == "sceinaro 3": #child & reset
            stool.turtlebot3_move=True   
            self.movebase_client(0,0,0) # 초기 위치
            stool.turtlebot3_move=False
                

        elif msg.data == "sceinaro 4": #adult & reset
            stool.turtlebot3_move=True   
            self.movebase_client(-0.2,0.9,0) # 사용자가 앉을 자리 옆
            stool.turtlebot3_move=False
